...
sub SB_PLAYER_Initialize {
    ...
    $hash->{AttrList}  .= "ftuiSupport:multiple-strict,1,0,medialist,favorites,playlists,genres,albums,albumsWithArtists,artists,all ";                       # CD 0065 neu # CD 0086 Auswahl hinzugefügt # CD 0103 genres, albums und artists hinzugefügt # CD 0104 all hinzugefügt
...
sub SB_PLAYER_Define {
    ...
    shift( @arguments ); # name
    shift( @arguments ); # type

    # needed for manual creation of the Player; autocreate checks in ParseFn
    if( SB_PLAYER_IsValidMAC( $arguments[ 0] ) == 1 ) {
        # the MAC adress is valid
        $hash->{PLAYERMAC} = lc($arguments[ 0 ]);       # CD 0026 lc added
    } else {
        # CD 0089 auf IP-Adresse prüfen
        if( SB_PLAYER_IsValidIPV4( $arguments[ 0] ) == 1 ) {
            $hash->{PLAYERMAC} = $arguments[ 0 ];
        } else {
            ...
            if($hash->{helper}{elapsedTime}{last} eq $args[ 0 ]) {
                ...
                if($hash->{helper}{elapsedTime}{count}>10) {
                    ...
                    if(ReadingsVal($name,"presence","x") eq "present") {
                        Log3( $hash, 4, "SB_PLAYER_Parse($name): currentTrackPosition frozen, player present, sending status request");
                        delete($hash->{helper}{disableGetStatus}) if defined($hash->{helper}{disableGetStatus});
                        SB_PLAYER_GetStatus( $hash );
                        $hash->{helper}{elapsedTime}{count}=0;
                    }
                    if(ReadingsVal($name,"presence","x") eq "absent") {
                        Log3( $hash, 3, "SB_PLAYER_Parse($name): currentTrackPosition frozen, player absent, trying to stop...");
...
sub SB_PLAYER_Set {
    ...
    # CD 0065 end
    # CD 0103 start
    } elsif( $cmd eq 'ftuicmd' ) {
        if( $arg[0] eq 'genre' ) {
            IOWrite( $hash, "$hash->{PLAYERMAC} playlistcontrol cmd:load genre_id:$arg[1]\n");
        } elsif( $arg[0] eq 'album' ) {
            IOWrite( $hash, "$hash->{PLAYERMAC} playlistcontrol cmd:load album_id:$arg[1]\n");
        } elsif( $arg[0] eq 'artist' ) {
            IOWrite( $hash, "$hash->{PLAYERMAC} playlistcontrol cmd:load artist_id:$arg[1]\n");
        }
    # CD 0103 end
    } else {
    ...
    if(defined($hash->{helper}{savedPlayerState}) && defined($hash->{helper}{savedPlayerState}{$statename})) {
        if($delonly==0) {
            ...
            if ((($hash->{helper}{savedPlayerState}{$statename}{power} eq "off") && ($forceon!=1))||($forceoff==1)) {  # CD 0036 added $forceon and $forceoff
                ...
                # CD 0101 nicht stoppen wenn anderer Player aus Gruppe zuvor abspielte
                if (defined($hash->{helper}{ttsPowerWasOffButOtherPlayerWasPlaying})) {
                    my @secbuf = split(',',AttrVal( $name, "fadeinsecs", '10,10' )); # CD 0050 split hinzugefügt
                    IOWrite( $hash, "$hash->{helper}{ttsPowerWasOffSendPlayCommand} play ".$secbuf[0]."\n" );
                    IOWrite( $hash, "$hash->{helper}{ttsPowerWasOffSendPlayCommand} time $hash->{helper}{savedPlayerState}{$statename}{elapsedTime}\n" ) if(defined($hash->{helper}{savedPlayerState}{$statename}{elapsedTime}));
                } else {
                    IOWrite( $hash, "$hash->{PLAYERMAC} stop\n" );  # CD 0091
                }
                IOWrite( $hash, "$hash->{PLAYERMAC} power 0\n" );
...
sub SB_PLAYER_Save {
    ...
    # CD 0101 start
    # falls Player aus ist, überprüfen ob andere Player in der Gruppe etwas abspielen
    delete($hash->{helper}{ttsPowerWasOffButOtherPlayerWasPlaying}) if(defined($hash->{helper}{ttsPowerWasOffButOtherPlayerWasPlaying}));
    if(($statename eq "xxTTSxx") && defined($hash->{helper}{ttsPowerWasOff})) {
        $hash->{helper}{savedPlayerState}{$statename}{power}="off";
        if (defined($hash->{SYNCGROUP}) && ($hash->{SYNCGROUP} ne '?') && ($hash->{SYNCMASTER} ne 'none')) {
            my @pl=split(",",$hash->{SYNCGROUP}.",".$hash->{SYNCMASTER});
            foreach (@pl) {
                my $pn=SB_PLAYER_MACToFHEMPlayername($hash,$_);
                if ($pn ne $name) {
                    if (ReadingsVal($pn,'playStatus','x') eq 'playing') {
                        Log3( $hash, 0, "SB_PLAYER_Save: $name: player $pn is playing, copying data") if defined($hash->{helper}{ttsOptions}{debugsaverestore});
                        $hash->{helper}{ttsPowerWasOffButOtherPlayerWasPlaying}=1;
                        $hash->{helper}{ttsPowerWasOffSendPlayCommand}=$_;
                        # path, playlistIds und playlistUrls 'borgen'
                        $hash->{helper}{path}=$defs{$pn}->{helper}{path};
                        $hash->{helper}{playlistIds}=$defs{$pn}->{helper}{playlistIds};
                        delete($hash->{helper}{playlistUrls}) if defined($hash->{helper}{playlistUrls});
                        if (defined($defs{$pn}->{helper}{playlistUrls})) {
                            foreach my $u ( keys %{$defs{$pn}->{helper}{playlistUrls}} ) {
                                $hash->{helper}{playlistUrls}{$u}=$defs{$pn}->{helper}{playlistUrls}{$u};
                            }
                        }
                    }
                }
...
sub SB_PLAYER_RecBroadcast {
    ...
    # CD 0103 start
        ...
        if($args[0] eq 'UPDATE') {
            my $data=uri_unescape($args[1]);
            my $ids=uri_unescape($args[2]);
            $hash->{helper}{genredata}=$data;
        ...
        if($args[0] eq 'UPDATE') {
            my $data=uri_unescape($args[1]);
            my $ids=uri_unescape($args[2]);
            my $artists=uri_unescape($args[3]); # CD 0107
            $hash->{helper}{albumdata}=$data;
...
# CD 0103 start
# ----------------------------------------------------------------------------
#  build list for FTUI medialist widget from genre, album or artist
# ----------------------------------------------------------------------------
sub SB_PLAYER_BAA2ftuiML {
    my ( $hash, $data, $ids, $data2 ) = @_;
    ...
    my @sids=split ':',$ids;
    my @sdata2;
    @sdata2=split ':',$data2 if defined($data2);  # CD 0107
    
    if (scalar @sdata != scalar @sids) {
        Log3( $hash, 2, "SB_PLAYER_BAA2ftuiML($name): array size mismatch (id-data)");
        return "";
    }
    # CD 0107
    if (defined($data2) && (scalar @sdata != scalar @sdata2)) {
        Log3( $hash, 2, "SB_PLAYER_BAA2ftuiML($name): array size mismatch (data-data2)");
        return "";
    }
    my $artist='';  # CD 0107
    for my $i (0 .. $#sdata) {
        $artist=$sdata2[$i] if defined($data2); # CD 0107
        if ($ftuireading eq "") {
            $ftuireading='[{"Artist":"' . $artist . '","Title":"' . $sdata[$i] . '","Album":"","Time":"","File":"'. $sids[$i] . '","Track":"","Cover":"/fhem/images/default/empty.png"}';
        } else {
            $ftuireading.=',{"Artist":"' . $artist . '","Title":"' . $sdata[$i] . '","Album":"","Time":"","File":"'. $sids[$i] . '","Track":"","Cover":"/fhem/images/default/empty.png"}';
        }
    }    
      ...
      if(( $hash->{ISREMOTESTREAM} eq "0" ) || ($hash->{ARTWORKURL} =~ /imageproxy%2F/)) {    # CD 0026 LMS 7.8/7.9
          ...
          if($hash->{ARTWORKURL} ne "?") {
              ...
              if ($hash->{ARTWORKURL} =~ /spotifyimage%2Fspotify/) {
                  ...
                  # CD 121 URL korrigiert
                  #$hash->{COVERARTURL} = "http://www.mysqueezebox.com/public/" .
                  $hash->{COVERARTURL} = 'http://' . $hash->{SBSERVER} .
                      '/imageproxy/' . $hash->{ARTWORKURL} . 
                      '/image_'.AttrVal( $name, 'coverartheight', 50 ).'x'.AttrVal( $name, 'coverartwidth', 50 ).'.jpg'.
                      '?x='.int(rand(100000));
              }
          } else {
              $hash->{COVERARTURL} = "http://" . $hash->{SBSERVER} . "/music/" .
